    ...
    export default {
        ...
        methods: {
                        ...
                        success: function (res) {
                            ...
                            wx.getLocalImgData({
                                ...
                                success: function (res) {
                                    ...
                                    self.axios.post(`${window.g.apiUrl}cetsearch/message/preset/ocr_base64`, form)
                                        .then(res => {
                                            if (res.data.status === 500) {
                                                ...
                                                if (data.username.length > 0 && data.admission_card_id.length == 15) {
            ...
            submitMessage: function () {
                let reg = /[\u4E00-\u9FFF]+/;
                if (reg.test(this.info.message.name) && this.info.message.ticket.length == 15) {
        ...
        mounted() {
            ...
            let has_message = window.location.search.substr(1).split('=');
            if (window.location.href === "https://wx.idsbllp.cn/cetsearch/login/redirect/preset") {
                formData.append('url', window.location.href);
                this.axios.post(`${window.g.apiUrl}cetsearch/setting/js_sign`, formData)
                    .then(res => {
                        let data = {
                            debug: false,
